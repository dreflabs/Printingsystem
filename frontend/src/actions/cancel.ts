...
export async function requestOrderCancellation(
  ...
  try {
    ...
    if (!actor.roles.includes("admin") && !actor.roles.includes("owner")) return fail("Hanya Admin/Owner yang boleh mengajukan pembatalan.");
...
export async function decideOrderCancellation(
  ...
  try {
    const tenant = await requireTenant();
    const actor = await requireMutableActor();
    if (!actor.roles.includes("owner")) return fail("Hanya Owner yang boleh memutuskan pembatalan.");
...
export async function cancelOrder(
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      } else if (!actor.roles.includes("owner")) {
