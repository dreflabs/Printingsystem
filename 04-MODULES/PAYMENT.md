...
- Alur umum: payment request → DP dibayar → konfirmasi Admin → PARTIAL → produksi mulai → pelunasan → PAID
- Aturan DP minimum berbeda untuk Walk-in (50% wajib) vs konsumen remote (bisa override oleh Admin/Owner)
- Mekanisme override DP dan pencatatan wajib di `audit_logs`
- Siapa yang boleh konfirmasi payment (hanya Admin) dan larangan role lain
