# Business Rules — Sistem Percetakan Print Pilot

## Aturan Bisnis Inti

1. Konsumen dapat datang langsung ke toko, menghubungi secara online (WA/media sosial), atau makloon (bawa file sendiri).
2. Admin atau Designer yang membuat order resmi di sistem — konsumen tidak punya akses ke sistem.
...
10. Final Audit wajib dilakukan sebelum order PRINTING bisa di-CLOSED. *(Pengecualian: pesanan `order_type = RETAIL` tidak melalui Final Audit — alurnya langsung `RETAIL_PAYMENT_COMPLETED → CLOSED` setelah barang diserahkan)*
...
13. DP minimum 50% mutlak untuk konsumen walk-in. Hanya Owner yang bisa memberikan override DP di bawah 50% (Admin DILARANG).
...
21. Penentuan **Harga Total** untuk order cetak kustom (PRINTING) dikalkulasi secara manual oleh Admin di luar sistem dan dimasukkan ke dalam form order. Sistem tidak memiliki *auto-calculate pricing engine* berdasarkan luas bahan.

---

## Aturan Deadline

- Peringatan otomatis H-1 (24 jam sebelum deadline) muncul di dashboard Owner dan Admin.
...
- Alert stok minimum dikirim ke Owner + Admin saat stok ≤ batas minimum.
...
- Admin dan Owner bisa menambahkan jenis bahan baru kapan saja langsung di sistem.
...
- Lebih dari 60 menit: alert ke Owner dan Admin.
...
- Pesanan RETAIL menggunakan "Fast-Track Workflow": `NEW_RETAIL_ORDER → RETAIL_PAYMENT_COMPLETED → CLOSED`.
- Tidak ada proses Desain, Produksi, QC, Finishing, Gudang, atau Final Audit untuk pesanan RETAIL.
- Stok barang jadi (`retail_products`) dipotong otomatis saat status mencapai `RETAIL_PAYMENT_COMPLETED`.
...
- Pembatalan pesanan RETAIL hanya bisa dilakukan sebelum pembayaran dikonfirmasi; setelah `RETAIL_PAYMENT_COMPLETED` dianggap final.
