...
export const PERMISSIONS = [
  ...
  "qc.submit", "finishing.execute", "storage.configure", "storage.store", "storage.move_to_counter", "storage.report_incident",
  ...
  "attendance.self", "attendance.view", "attendance.configure", "payroll.view", "payroll.manage", "report.view_operational", "report.view_financial", "report.export", "shop.configure",
] as const;
...
const MUTATING = new Set<Permission>([
  ...
  "production.assign", "production.reassign", "production.execute", "production.input_material", "production.submit_waste", "production.rework_decide",
  "qc.submit", "finishing.execute", "storage.configure", "storage.store", "storage.move_to_counter", "storage.report_incident", "audit.submit", "audit.approve",
  ...
  "user.reset_password", "attendance.self", "attendance.configure", "payroll.manage", "shop.configure",
]);
...
const ROLE_PERMISSIONS: Record<string, ReadonlySet<Permission>> = {
  ...
  admin: new Set<Permission>([
    ...
    "design.upload", "design.approve_online", "design.request_revision", "production.assign", "production.reassign", "qc.submit", "storage.configure", "storage.report_incident",
    "audit.submit", "correction.create_operational", "report.view_operational", "report.view_financial", "report.export", "attendance.self", "attendance.view",
  ]),
  ...
  operator: new Set<Permission>([
    "order.view_scoped", "production.execute", "production.input_material", "production.submit_waste", "attendance.self",
  ]),
  gudang: new Set<Permission>([
    "order.view_scoped", "qc.submit", "finishing.execute", "storage.store", "storage.move_to_counter", "storage.report_incident", "attendance.self",
  ]),
};
