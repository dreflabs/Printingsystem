...
- **MFA wajib** untuk login akun `super_admins` — mengingat akun ini punya jangkauan akses ke *seluruh* tenant sekaligus, jauh lebih sensitif dari akun Owner tenant manapun yang cuma bisa akses data tenant sendiri. **Terpasang:** kode OTP 6 digit dikirim ke **email** akun tiap login (tanpa aplikasi authenticator). Login 2 langkah: email+password → kode dikirim → masukkan kode (berlaku 10 menit, salah 5× dibatalkan). Butuh `MAIL_PROVIDER` aktif; break-glass `PLATFORM_OTP_DEBUG=1` menaruh kode di log server. Sesi panel dibatasi 12 jam (`PLATFORM_SESSION_MAX_AGE_MS`). Pemulihan: SUPER_ADMIN lain reset password, atau `bootstrap:superadmin` di server.
