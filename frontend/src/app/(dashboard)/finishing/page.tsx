...
const KPI = [
  ...
  { label: "Sedang Dikerjakan", value: "1", color: "text-accent-purple", bg: "bg-accent-purple/10", icon: Wrench },
      ...
      <div className="relative w-full max-w-sm bg-card border border-border rounded-2xl shadow-[0_8px_48px_rgba(0,0,0,0.6)] p-6">
        ...
        <div className="bg-white text-black rounded-xl p-4 mb-5 border-2 border-dashed border-border">
          <div className="text-center mb-3">
            <p className="text-sm font-bold text-black">🖨️ PrintFlow Percetakan</p>
            <p className="text-xs text-gray-500">Jl. Contoh No. 123 · Telp: 0800-xxxx</p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs font-semibold text-black mt-1 truncate">{job.product}</p>
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-muted mt-0.5">Antrian job QC PASSED siap di-finishing</p>
      </div>
      ...
      {/* Active Job */}
      {activeJob ? (
        <div className="bg-gradient-to-br from-accent-purple/10 to-accent-teal/5 border border-accent-purple/30 rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-3">
            <span className="h-2 w-2 rounded-full bg-accent-purple animate-pulse" />
            <span className="text-xs font-semibold text-accent-purple uppercase tracking-wide">Finishing Berjalan</span>
          </div>
          ...
          {!showDoneForm ? (
            <button
              ...
              className="w-full h-14 rounded-xl bg-gradient-to-r from-accent-purple to-blue-500 text-white text-base font-bold hover:brightness-110 transition-all cursor-pointer"
            >
              🔧 SELESAI FINISHING
            </button>
          ) : (
            <div className="space-y-3">
              <input type="number" value={qty} onChange={(e) => setQty(e.target.value)}
                ...
                className="w-full h-12 rounded-xl bg-card border border-border text-primary text-lg font-bold px-4 outline-none focus:border-accent-purple transition-all" />
              <button
                disabled={!qty}
                onClick={() => { setShowDoneForm(false); setLabelFor(activeJob); }}
                className="w-full h-12 rounded-xl bg-accent-purple text-white text-sm font-bold hover:brightness-110 transition-all cursor-pointer disabled:opacity-40"
              >
                Submit & Cetak Label
              </button>
            </div>
      ...
      {/* Scan CTA */}
      <button className="w-full h-14 rounded-xl bg-elevated border-2 border-dashed border-accent-purple/40 text-accent-purple font-semibold flex items-center justify-center gap-2 hover:bg-accent-purple/10 transition-all cursor-pointer">
          ...
          {queueJobs.map((j, i) => {
            ...
            <div key={j.id} className={cn("flex items-center gap-4 p-4 hover:bg-elevated/30 transition-colors")}>
              ...
              <button
                ...
                className="shrink-0 h-10 px-4 rounded-xl bg-accent-purple/20 border border-accent-purple/40 text-accent-purple text-xs font-bold hover:bg-accent-purple/30 transition-all cursor-pointer"
              >
                Mulai
              </button>
            </div>
